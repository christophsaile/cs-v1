...
{
	const init = () => {
		const elementsWithDataInit: HTMLElement[] = [].slice.call(
			document.querySelectorAll("[data-init]")
		);
	...
	const setupResourceLoader = (callback: Function) => {
		...
		return new ResourceLoader({
			...
			baseMap: {
				"##content": biotope.configuration.get(
					"data.staticResourcesContentRepoBase"
				)
			},
